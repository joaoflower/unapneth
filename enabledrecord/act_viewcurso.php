...
<?
	...
	if(fsafetylogin())
	{
		...
		$bData = FALSE;
		$vNombres = "";
		$_SESSION['sActacod_prf'] = $_POST['rCod_prf'];
		$_SESSION['sActanom_prf'] = "";
		...
		{
			$bData = TRUE;
			$_SESSION['sActanom_prf'] = "{$aResult['paterno']} {$aResult['materno']}, {$aResult['nombres']}";
		}
...
<center>
	<table border="0" cellpadding="0" cellspacing="0" class="tventana">
	  <tr>
		...
		<th background="../images/ven_topcenter.jpg">Cursos de: <?=$_SESSION['sActanom_prf']?></th>
	  ...
	  <tr>
		...
		<td align="center">
			...
			$vQuery .= "from $tCarga si left join curso cu on si.cod_car = cu.cod_car and ";
			...
			$vQuery .= "where si.per_aca = '{$_SESSION['sFrameper_aca']}' and si.cod_prf = '{$_POST['rCod_prf']}' ";
			...
			?>
				<tr <?=ftrstyle($vCont)?>  onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
				  ...
				  <td <?=ftdstyle($vNum_rows, $vCont)?>><?=ucwords(strtolower($aResult['car_des']))?></td>
			  	  <td <?=ftdstyle($vNum_rows, $vCont)?>><a href="" onclick="act_viewcursodata('<?=$aResult['cod_car']?>', '<?=$aResult['pln_est']?>', '<?=$aResult['cod_cur']?>', '<?=$aResult['sec_gru']?>', '<?=$aResult['mod_mat']?>'); return false;" class="enlaceb"><img src="../images/browse.png" alt="Ver Informacion del Curso" width="16" height="16" /></a></td>
				</tr>
			<? 
				$vCont++; 	
			} 
