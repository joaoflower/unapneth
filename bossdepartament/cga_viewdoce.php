...
<?
	...
	$vQuery = "Select cod_prf, cod_car, paterno, materno, nombres from docente where cnd_act = '1' and cod_car = '{$_SESSION['sUsercod_car']}' ";

	if((!empty($_POST['rPaterno']) or !empty($_POST['rMaterno']) or !empty($_POST['rNombres'])))
	{		
	...
	{	
		...
		?>
			<tr <?=ftrstyle($vCont)?>  onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
			  ...
			  <td <?=ftdstyle($vNum_rows, $vCont)?> width="16"><a href="" onclick="cga_getcarrera('<?=$aResult['cod_prf']?>'); return false;" class="enlaceb"><img src="../images/browse.png" alt="Ver datos de Docente" width="16" height="16" /></a></td>
			</tr>
		<? 
			$vCont++; 	
		} 
